...
export class ServexService {
  ...
  /**
   * Suspende o reactiva un cliente en Servex
   * Este endpoint es un toggle: si está activo lo suspende, si está suspendido lo activa.
   */
  async suspenderCliente(clienteId: number): Promise<any> {
    try {
      console.log(`[Servex] Cambiando estado de suspensión para cliente ID ${clienteId}`);
      const response = await this.client.put(`/clients/${clienteId}/suspend`);
      console.log(`[Servex] ✅ PUT /clients/${clienteId}/suspend - ${response.status}`);
      return response.data;
    } catch (error: any) {
      const mensaje = error.response?.data?.message || error.message;
      console.error("[Servex] ❌ Error suspendiendo/activando cliente:", mensaje);
      throw new Error(`Error en Servex suspendiendo cliente: ${mensaje}`);
    }
  }

  /**
   * Activa o desactiva un revendedor y toda su jerarquía
   * También funciona como un toggle.
   */
  async toggleStatusRevendedor(revendedorId: number): Promise<any> {
    try {
      console.log(`[Servex] Toggling status para revendedor ID ${revendedorId}`);
      const response = await this.client.put(`/resellers/${revendedorId}/toggle-status`);
      console.log(`[Servex] ✅ PUT /resellers/${revendedorId}/toggle-status - ${response.status}`);
      return response.data;
    } catch (error: any) {
      const mensaje = error.response?.data?.message || error.message;
      console.error("[Servex] ❌ Error toggling status revendedor:", mensaje);
      throw new Error(`Error en Servex toggling status revendedor: ${mensaje}`);
    }
  }
